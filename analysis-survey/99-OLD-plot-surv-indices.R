...
stopifnot(nrow(anti_join(dat, stock_df, by = join_by(stock))) == 0L)

dat <- left_join(dat, stock_df, by = join_by(stock))
...
all_indices <- left_join(all_indices, slopes, by = join_by(species, region))
...
# combine slopes where there are multiple surveys included:
...
d5 %>% select(stock_clean, species, gear_simple, index, surveys, year, est, lwr, upr) %>%
