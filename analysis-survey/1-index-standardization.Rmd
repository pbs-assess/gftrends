...
if (region != "HBLL inside surveys") {
  all_data <- readRDS(here::here("data-generated/all-survey-sets-2021.rds")) %>%
    filter(species_common_name == tolower(species)) %>%
    rename(survey_series_id = survey_series_id.x)
} else {
  all_data <- readRDS(here::here("data-generated/all-inside-hbll-2021.rds")) %>%
    filter(species_common_name == tolower(species))
}
```
...
if(region %in% c("HBLL inside surveys")) {
  cutoff <- 10
  update_model <- TRUE
}
...
if (delta_model) {
  try({
    m2 <- readRDS(here::here("data-generated/models/m-", spp, "-", ssid_string, name, "-gamma.rds"))
  })
  if (!exists("m2")) update_model <- TRUE
} else {
  try({
    m <- readRDS(here::here("data-generated/models/m-", spp, "-", ssid_string, name, ".rds"))
  })
...
if (update_model) {
  ...
  if (delta_model) {
    ...
    tictoc::toc()
    saveRDS(m1, file = here::here("data-generated/models/m-", spp, "-", ssid_string, name, "-presence.rds"))
...
# if (delta_model) {
try({
  m1 <- readRDS(here::here("data-generated/models/m-", spp, "-", ssid_string, name, "-presence.rds"))
...
Get simulated index
```{r}
dir.create("data-generated/indices", showWarnings = FALSE)
...
if (delta_model & exists("m1") & exists("m2")) {
  ...
  if (both_converged & update_index) {
    ...
    saveRDS(i, here::here("data-generated/indices/i-", spp, "-", ssid_string, name, "-delta.rds"))
  }
} else {
  if (exists("m")) {
    if (max(m$gradients) < 0.1 & !m$bad_eig & m$sd_report$pdHess & update_index) {
      ...
      i <- left_join(i, samples)
      saveRDS(i, here::here("data-generated/indices/i-", spp, "-", ssid_string, name, ".rds"))
    }
  }
}
```
...
if (delta_model) {
  try({
    i <- readRDS(here::here("data-generated/indices/i-", spp, "-", ssid_string, name, "-delta.rds"))
  })
} else {
  try({
    i <- readRDS(here::here("data-generated/indices/i-", spp, "-", ssid_string, name, ".rds"))
  })
}
