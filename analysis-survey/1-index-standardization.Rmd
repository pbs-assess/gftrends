...
if (region == "HBLL outside surveys") {
  ...
  d_N <- all_data$survey_sets %>% filter(survey_abbrev == "HBLL OUT N") 
  timeN <- sort(unique(d_N$year))
  nd_N2 <- do.call("rbind", replicate(length(timeN), nd_N, simplify = FALSE))
  nd_N2[["year"]] <- rep(timeN, each = nrow(nd_N))
  
  nd_S <- gfplot::hbll_s_grid$grid %>% mutate(survey = "HBLL OUT S")
  d_S <- all_data$survey_sets %>% filter(survey_abbrev == "HBLL OUT S") 
  timeS <- sort(unique(d_S$year))
  nd_S2 <- do.call("rbind", replicate(length(timeS), nd_S, simplify = FALSE))
  nd_S2[["year"]] <- rep(timeS, each = nrow(nd_S))
  nd_all <- bind_rows(nd_N2, nd_S2) %>% rename(longitude = X, latitude = Y) %>% mutate(area = 4)
