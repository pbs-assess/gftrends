...
if (region == "HBLL outside surveys") {
  nd_all <- readRDS(paste0("data/nd_hbll_outside_index.rds")) 
...
if (region == "HBLL inside surveys") {
  nd_all <- readRDS(paste0("data/nd_hbll_inside_index.rds")) 
...
all_data <- readRDS("~/github/dfo/gftrends/analysis-survey/data/all-survey-sets-2021.rds") %>% 
  filter(species_common_name == tolower(species)) %>% rename(survey_series_id=survey_series_id.x)
} else {
all_data <- readRDS("~/github/dfo/gftrends/analysis-survey/data/all-inside-hbll-2021.rds") %>%
...
if (delta_model) {
  ...
  if (prop_pos < 0.1) {
    spde2 <- make_mesh(data2, xy_cols = c("X", "Y"), cutoff = 25)
  } else {
    if (prop_pos < 0.7) {
      spde2 <- make_mesh(data2, xy_cols = c("X", "Y"), cutoff = 10)
    } else {
      spde2 <- make_mesh(data2, xy_cols = c("X", "Y"), cutoff = 15)
    }
  }
# }
}
...
if (delta_model) {
  try({ m2 <- readRDS(paste0("models/m-", spp, "-", ssid_string, name, "-gamma.rds"))})
  if (!exists("m2")) update_model <- TRUE
}else{  
  try({ m <- readRDS(paste0("models/m-", spp, "-", ssid_string, name, ".rds"))})
...
if (update_model) {
  ...
  if (!all(sort(unique(data$year))==sort(unique(data2$year)))) {
    ...
    missing_years <- anti_join(data, pos_years, by = "year") %>% select(year) %>% unique()
  ...
  tictoc::toc()
  saveRDS(m2, file = paste0("models/m-", spp, "-", ssid_string, name, "-gamma.rds"))

  } else { delta_model <- FALSE }  
  
  if(!m2$sd_report$pdHess | m2$bad_eig){delta_model <- FALSE} 
...
```{r}
# if (delta_model) {
    try({ m1 <- readRDS(paste0("models/m-", spp, "-", ssid_string, name, "-presence.rds"))
...
try({m1})
try({max(m1$gradients)})
try({paste("Hessian positive and definite =", m1$sd_report$pdHess)}) 
try({paste("Bad eigen =", m1$bad_eig)}) 
try({tidy(m1, effects = "ran_pars", conf.int = TRUE)})

try({m2})
try({max(m2$gradients)})
try({paste("Hessian positive and definite =", m2$sd_report$pdHess)}) 

try({paste("Bad eigen =", m2$bad_eig)}) 
try({tidy(m2, effects = "ran_pars", conf.int = TRUE)})

try({m})
try({max(m$gradients)})
try({paste("Hessian positive and definite =", m$sd_report$pdHess)}) 
try({paste("Bad eigen =", m$bad_eig)}) 
try({tidy(m, effects = "ran_pars", conf.int = TRUE)})
```
...
if (delta_model & exists("m1") & exists("m2")) {
    ...
    saveRDS(i, paste0("indices/i-", spp, "-", ssid_string, name, "-delta.rds"))
  }
} else {
if (exists("m")) {
...
if (delta_model) {
try({
  i <- readRDS(paste0("indices/i-", spp, "-", ssid_string, name, "-delta.rds"))
})
} else {
try({
