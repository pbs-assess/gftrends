...
if(species=="Shortbelly Rockfish"){
  all_data <- readRDS(here::here("data-raw/shortbelly.rds")) 
}
```
...
if (update_model) {
  ...
  if (delta_model) {
    ...
    if (m1$sd_report$pdHess && !m1$bad_eig) {
      ...
      if (length(missing_years2) == 0L) missing_years <- NULL
...
try({
  ...
  if (max(m1$gradients) > 0.01) {
    ...
    saveRDS(m1, file = here::here(paste0("models/m-", spp, "-", ssid_string, name, "-presence.rds")))
  }
})
try({
  ...
  if (max(m2$gradients) > 0.01) {
    ...
    saveRDS(m2, file = here::here(paste0("models/m-", spp, "-", ssid_string, name, "-presence.rds")))
  }
})
# } else {
...
if (delta_model) {
  fitted_yrs <- sort(union(unique(m1$data$year), unique(m2$data$year)))
} else {
...
if (delta_model && exists("m1") && exists("m2")) {
  ...
  if (both_converged && update_index) {
    p1 <- predict(m1, newdata = nd, nsim = 1000)
    p2 <- predict(m2, newdata = nd, nsim = 1000)
  ...
  if (exists("m")) {
    if (max(m$gradients) < 0.1 && !m$bad_eig && m$sd_report$pdHess && update_index) {
      p <- predict(m, newdata = nd, nsim = 1000)
      i <- get_index_sims(p, area = nd$area)
      i <- i %>% mutate(
        species = species,
        type = type,
...
if (exists("i")) {
  ...
  ggsave(here::here(paste0("figs/", spp, "-", ssid_string, name, if (delta_model) "-delta", ".png")), height = 4, width = 6)
  g
}
```
