...
all_indices <- readRDS(paste0("analysis-survey/data/sopo-2021-indices.rds")) %>%
  rename(gear = type)
dat <- readRDS("data-generated/b-status-dat.rds") %>% filter(year >= 2000) %>%
  rename(dfo_area = region) %>% select(-species) %>% group_by(stock) %>%
  mutate(mean_blrp = mean(exp(log_blrp), na.rm = T)) %>% ungroup()
dat <- left_join(dat, stock_df)
d <- left_join(all_indices, stock_df, by = c("species", "region")) %>% rename(index = region, model_type = model) %>% mutate(model = paste(index, model_type))
d <- full_join(dat, d)

d1 <- filter(d, !is.na(stock_clean)) %>% group_by(stock_clean, index, gear, model) %>%
  mutate(mean_est = mean(est, na.rm = T)) %>% ungroup()
...
ggplot(d2) +
  geom_ribbon(aes(year, ymin = q0.05_blrp/mean_blrp,  ymax = q0.95_blrp/mean_blrp,
                  group = stock_clean), fill = "black", alpha = 0.2) +
  ...
  geom_ribbon(aes(year, ymin = lwr / mean_est, ymax = upr / mean_est,
                  group = model, fill = gear), alpha = 0.3) +
  ...
  ggsidekick::theme_sleek() + theme(axis.text.y = element_blank(),
...
ggplot(d2, aes(year, est / mean_est), group = index, fill = gear, colour = gear) +
  ...
  geom_ribbon(aes(ymin = lwr / mean_est, ymax = upr / mean_est,
                  group = index, fill = gear), alpha = 0.4) +
  ylab("Biomass (tonnes)") +
