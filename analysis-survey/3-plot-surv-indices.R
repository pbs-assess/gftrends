...
dat <- readRDS("data-generated/b-status-dat.rds") %>%
  ...
  select(-species) %>%
  group_by(stock) %>%
  mutate(mean_blrp = exp(mean(log_blrp, na.rm = TRUE)),
...
d2 %>%
  ...
  geom_line(aes(year, est / mean_est, group = model)) +
  geom_ribbon(aes(year,
  ...
  scale_fill_brewer(name = "Index type", palette = "Dark2", na.value="black") +
