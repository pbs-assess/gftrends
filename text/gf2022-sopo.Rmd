...
* Stock assessments classified three stocks (Strait of Georgia Lingcod [Area 4B] and inside and outside Quillback Rockfish) as having had a $>$ 5% probability of being in the critical zone. Conversely, two-thirds of assessed stocks had a high (\>75%) probability of being in the "healthy zone".

* All assessed shelf rockfish (Bocaccio, Canary, Redstripe, Silvergray, Widow,  Yellowtail) and several slope rockfish increased in surveyed biomass over the  last ~5--7 years. Surveyed biomass for several flatfish (Petrale, English, Rex, and Dover Sole) increased over the last 5–-10 years but declined for Arrowtooth Flounder.
...
We estimated the overall mean B/LRP in 2020 to be `r make_quant_text(q2021lrp)`.
The overall mean B/USR and B/B~MSY~ in 2020 was `r make_quant_text(q2021usr)` and `r make_quant_text(q2021msy)`, respectively (Figure \@ref(fig:pa-framework), \@ref(fig:ridges)).
...
Vertical dashed lines are at values of 1.0 in all columns and also at values of 0.4 and 0.8 in the B/B~MSY~ column (0.4 and 0.8 B~MSY~ are provisional LRP and USR values).
The x-axis has been square-root transformed to slightly compress high ratio values for visualization.
In a few cases (e.g., Bocaccio) the status represents a one- or two-year projection from the last year of available data [@dfo2020bocaccio].
The full posteriors were not available for Quillback or Canary Rockfish stocks, which are shown in Figure \@ref(fig:stock-vs-indices) [@yamanaka2011quillback; @stanly2009canary].
...
Dark grey lines and ribbons represent output from stock assessments: trajectories of median B/LRP 95% quantile credible intervals (see citations in Figure \@ref(fig:ridges) caption).
...
Survey indices for all assessed shelf rockfish (Bocaccio, Canary, Redstripe, Silvergray, Widow, Yellowtail), and some slope rockfish (e.g. Yellowmouth) increased in the past 5--7 years (Figure \@ref(fig:stock-vs-indices)).
...
On the other hand, after decades of consistently low recruitment, recent increases in several shelf rockfishes species---most notably Bocaccio---may in part by driven by availability of oxygen-rich water at depth during gestation [@schroeder2019; @dfo2022bocaccio].
