...
* All the usual groundfish surveys were completed this year, in addition to
  those that were missed last year due to covid.

<!-- TODO: check/update numbers -->
* To assess stock status in 2021, we took two approaches: 1) we updated the
  hierarchical Bayesian state-space time-series model to include **6** newly
  complete stock assessments, and 2) we estimated model-based indices from the
  relevant surveys. We then scaled scaled the survey indices to existing
  assessments based on the geometric mean in overlapping years to illustrate
  how stock status may be changing subsequent to the most recent completed
  stock assessments.
...
* Same as in 2020, stock assessments suggest that 4 or 5 stocks were likely in
  the critical zone, while roughly one-third of stocks had a greater than 1 in
  4 chance of being in the "cautious zone". Conversely, two-thirds of assessed
  groundfish stocks had a high (\>75%) probability of being in the "healthy
  zone".

* All assessed shelf rockfish (Redstripe, Bocaccio, Widow, Silvergray, Canary,
  Yellowtail) and some slope rockfish have increased over last ~5–6 years,
  possibly due to extreme/spasmodic recruitment.

* Many flatfish (Petrale, English, Rex, Dover) have increased over last 5–10
  years (but Arrowtooth has declined).

* Dogfish stocks have experienced the steepest declines of all stocks with the
  steepest decline being found in inside Vancouver Island waters in 2021.

* An update to A DATA SYNOPSIS FOR BRITISH COLUMBIA GROUNDFISH will be released
  as a Science Response in 2022 and will provide additional information of
  population and fishing trends, growth and maturity patterns, and data
  availability for 113 fish species.
...
We fit geostatistical models of biomass density with TMB [@kristensen2016] via the R package sdmTMB [@anderson2022].
These models account for all latent spatial factors with a constant spatial Gaussian random field and allow spatiotemporal deviations to evolve as a Guassian random field random walk.
The random fields were structured with Matern correlation functions.
For each stock--survey type combination, we fit both Tweedie and a delta-gamma (binomial and Gamma) observation error models and report the model that produced the most precise biomass estimates on average.
In most cases this was the delta-gamma.
We predicted each biomass index for the appropriate 2 x 2km survey grid(s) and scaled the survey index to the existing assessment biomass trend based on the geometric mean in overlapping years.
...
Survey indices for all assessed shelf rockfish (Bocaccio, Canary, Redstripe, Silvergray, Widow, Yellowtail), and some slope rockfish (e.g. Yellowmouth) have increased in the past 5--7 years (Figure \@ref(fig:stock-vs-indices)), possibly due to a shared spasmodic recruitment event (**REF?**).
...
The majority of the flatfish (Petrale, English, Rex, Dover) appear to have increased in survey biomass over the last 5--10 years.
However, the survey index suggests that Arrowtooth has declined since the last assessment (Figure \@ref(fig:stock-vs-indices)).
Trends among the skates generally appear stable or positive.
North Pacific Spiny Dogfish stocks, however, have experienced the steepest declines of all stocks (Figure \@ref(fig:stock-vs-indices)), with a particularly steep decline having been observed in inside Vancouver Island waters in 2021.
...
One possibility is that climate change is driving a northward range shift or that seasonal distribution patterns have changed (REF).
...
After decades of consistently poor recruitment, recent increases in several shelf rockfishes species---most notably Bocaccio---may in part by driven by availability of oxygen-rich water at depth during gestation [@schroeder2019].
...
Four or five stocks are likely below their LRP.
